...
export default function ReviewerDashboard() {
  ...
  const loadAssignments = async () => {
    ...
    setError(null);
    try {
      const data = await reviewerService.getMyAssignments();
  ...
  const handleOpenSection = (assignment) => {
    if (assignment.tender_type === 'platform') {
      navigate(`/reviewer/proposal/${assignment.proposal_id}/section/${assignment.section_id}`);
    } else {
      navigate(`/reviewer/uploaded/${assignment.uploaded_tender_id}/section/${assignment.section_key}`);
    }
  };
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-slate-600 mt-1">
          ...
          Here are your assigned sections to review
        </p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-200">
          ...
          <p className="text-sm text-slate-500 mt-1">
            Sections assigned to you for review
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-slate-500 max-w-md mx-auto">
              ...
              When a bidder assigns you to review a section, it will appear here.
            </p>
          </div>
        ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className={`px-2 py-0.5 text-xs rounded ${
                        assignment.tender_type === 'platform'
                          ? 'bg-blue-100 text-blue-700'
                          : 'bg-purple-100 text-purple-700'
                      }`}>
                        {assignment.tender_type === 'platform' ? 'Platform' : 'Uploaded'}
                      </span>
                    </div>

                    {/* Section Title */}
                    <p className="text-slate-600 mb-2">
                      Section: <span className="font-medium">{assignment.section_title || assignment.section_key}</span>
                    </p>
